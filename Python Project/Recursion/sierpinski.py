...
def main():
	"""
	This program can draw a sierpinski triangle.
	"""
...
def sierpinski_triangle(order, length, upper_left_x, upper_left_y):
	"""
	:param order: (int) Control the the order of Sierpinski Triangle.
	...
	# Recursive case.
		...
		left_l = GLine(upper_left_x, upper_left_y, upper_left_x + length * 0.5, upper_left_y + length * 0.866)
		right_l = GLine(upper_left_x + length, upper_left_y, upper_left_x + length * 0.5, upper_left_y + length * 0.866)
		window.add(upper_l)
		window.add(left_l)
		window.add(right_l)
